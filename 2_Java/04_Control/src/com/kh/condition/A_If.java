...
public class A_If {
	...
	public void method1() {
		...
		// 입력받은 성적이 60점 이상이면 "합격입니다"를 출력
			...
			// 한 줄만 들어갔을 때 유용! / 두개 이상의 결과문을 쓸려면 중괄호{}안에 써야함
			// if(score >= 60) 
			// System.out.println("합격입니다");
	...
	public void method3() {
		...
		System.out.println("본인이름의 주소값 : " + System.identityHashCode("이준용"));
	 ...
	 *   조건식1이 참(true)일 때 실행
	 * } else if(조건식2) {
	 *   조건식1이 거직(false)이면서 조건식2가 참(true)일 때 실행
	 * } else {
	...
	public void method4() {
		...
		if(number > 0) {
			...
			if (number == 0) { // if문 중첩 가능! 근데 가급적으로는 좋은 코드는 아니다
		...
		if (number > 0) {
			System.out.println("양수");
		} else if(number == 0) { // if문 중첩 가능! 근데 가급적으로는 좋은 코드는 아니다
				System.out.println("0이다");
			} else {
				System.out.println("음수");
			}
	...
	/*
	 * 사용자에게 점수(0~100)를 입력받아서 전구별로 등급 출력
	 * - 90점 이상은 A 등급
	 * - 90점 미만 80점 이상은 B 등급
	 * - 80점 미만 70점 이상은 C 등급
	 * - 70점 미만 60점 이상은 D 등급
