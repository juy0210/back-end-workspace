...
public class C_For {
	...
	public void method3() {
		...
		// 다른 방법 : 1 과 10 사이에서 1부터 2를 더한 숫자만 출력
	...
	public void method6() {
		 ...
		 * 호출할 때마다 매범 다른 랜덥값을 얻어낼 수 있음
		 * */
		double random = Math.random(); // 0.0 <= random < 1.0 만 해당
		random = Math.random() * 10; // 0.0 <= random < 10.0
		random = Math.random() * 10 + 1; // 1.0 <= random < 11.0
		
		int num = (int)random; // 
		System.out.println(); // int는 소수점 제외한 정수만 입력해줌
	}
	...
	public void method7() {
		 ...
		 * }
		 * 
		 * 문자열 : 문자의 배열, 열 개의 문자가 배열을 이룬 것이 문자열
