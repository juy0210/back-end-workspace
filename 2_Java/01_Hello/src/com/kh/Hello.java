 ...
 *   - 운영체제에 독립적이다.(JVM)
 *   - 네트워크와 부누산처리를 지원한다. (Java API)
 *   - 멀키 스레드를 지원한다. (Java API)
 *   - 동적 로딩을 지원한다.
 *   
 * * JDK > JRE > JVM
 ...
 *   - 번역된 바이트코드(기계어)를 JVM JVM의 자바 인터프리터(java.exe)를 이용해 실행한다.
 *   
 * * 패키지(package)
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	 ...
	 *                      회사의 도메일 이름으로 패키지를 만든다.
	 * - 클래스 : 대문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기 ex) HelloWorld
	 * - 메서드 : 소문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마자 대문자로 표기                    
	 * */
