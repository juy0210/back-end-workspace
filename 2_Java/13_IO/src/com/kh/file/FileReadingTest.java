...
//		f.fileRead2();
	...
	public void fileSave() {
		...
		try(FileWriter fw = new FileWriter(fileName, false)) { // import string or fileName 입력
			// true - 이어서 쓰기, false = 새롭게 쓰기
			fw.write("IO\n");
			fw.write("Thread\n");
			fw.write("Network\n");
			
			fw.flush(); // 강제로 자료 출력 > 실행 > 프로젝트 refresh 하면 파일 생성
			            // 다시 실행 > test > 추가적으로 같은 내용 입력됨
		} catch(IOException e) {
			e.printStackTrace();
		}
	...
	// 보조 스트림 BufferedReader : 버퍼(char[])에 데이터가 쌓이기를 기다렸다가 꽉 파게 될 시 한꺼번에 보냄
	...
	public void fileReadToSave() {
		 ...
		 * PrintWriter : 프린트와 유사하게 출력하는 print(), println() 메서드를 가지고 있는 보조스트림
		 * */
		try(
