...
public class A_TryCatch {
	 ...
	 * 런타임 에서(runtime error) : 실행 시에 발생하는 에러
	 * 논리적 에서(logical error) : 실행은 되지만, 의도와 다르게 동작
	 * 
	 * 런타임 에서 두 가지!
	 ...
	 * 
	 * Exception -->
	 *                 IOException - FileNotFoundException, SockeyException
	 ...
	 * } catch(예외클래스 예외변수명) {
	 * 
	 *      // try에서 예외가 발생하면 실행을 멈추고 catch로 들어와서 예외 처리코드 실행
	 * 
	 * } finally {
	 * 
