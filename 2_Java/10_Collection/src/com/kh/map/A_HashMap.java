...
public class A_HashMap {
	...
	public void method1() {
		...
		map.put("고래밥", 180);
		map.put("칸쵸", 800);
		map.put("나쵸", 450); // {웨하스=250, 고래밥=180, 나쵸=450, 칸쵸=800}
		//     키(key) X, 값(value) O
		...
		System.out.println(key); // [웨하스, 고래밥, 나쵸, 칸쵸] 앞에 key 부분만 가져온다
		
		// 2. 값만 가져오기
		Collection<Integer> col = map.values();
		System.out.println(col); // [250, 180, 450, 800] 뒤에 값만 가져온다
		
		// 3. 키에 해당하는 value 값 가져오기
	    for(String s : key) {
	    	System.out.println(s + " : 0 " + map.get(s));
	    	/*
	    	 * 웨하스 : 0 250
             * 고래밥 : 0 180
             * 나쵸 : 0 450
	...
	public void method2() {
		...
		System.out.println(map); // 꼬깔콘 없어짐
