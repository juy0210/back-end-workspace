...
//		list.add(3);
	...
	public void method2() {
		...
		// 4. size : 리스트 안에 몇개의 데이터가 있는지 확인
		System.out.println("사람 수 : " + list.size()); // 6 = 6명
		
		// 5. remove : 해당 인덱스의 객체 삭제
		//     -> 알아서 크기 줄어들고 뒤에 객체가 앞으로 다 땅겨옴
		list.remove(0);
		System.out.println("삭제 후 사람 수 : " + list.size()); // 삭제 후 사람 수
		
		// 6. get : 해당 인덱스의 객체 가져오기
		System.out.println(list.get(0)); // 0번째만 가져옴
	...
	public void method3() {
		...
		// 11. 오름차순 정렬 : Comparable 인터페이스를 구형하고 있는 요소를 가지고 비교값들을 반환하여 정렬
		Collections.sort(list);
		
		// 12. 내림차순 정렬 : sort 메서드를 통해서 오름차순 정렬 후 
		//                 reverse 메서드를 호출하여 정렬 > sort를 먼저 써야 사용 가능하다
