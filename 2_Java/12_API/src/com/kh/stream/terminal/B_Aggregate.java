...
public class B_Aggregate {
	...
	public void method1() {
		...
		/*
		 * Optional 클래스
		 * - 스크림의 최종 결과 값을 저장하는 객체
		 * - 단순히 값만 저장하는 것이 아니라, 값의 존재 여부를 확인하고
		 *   값이 존재하지 않을 결루 디폴트 값을 설정할 수 있는 객체
		 * */
		...
		System.out.println("평균 : " + avgResult); // 평균 : 3.5
		
		OptionalInt max = Arrays.stream(array).max();
		// orElse : get과 동일하게 저장되어 ㅣㅇㅆ는 값을 얻어오는 메서드로
		//          값이 저장되어 있지 않을 경우 티폴트 값을 지정
		int maxResult = max.orElse(0);
		System.out.println("최대값 : " + maxResult); // 최대값 : 6
