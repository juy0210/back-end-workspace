...
/*
 * 스트림(stream)
 * - 다양한 데티어 소스를 표준화된 방법으로 다루기 위한 것
 * 
 * * 스트림의 특징
 ...
 * 
 * * 스트림의 종류
 * - java.util.stream 패키지에 존재하고 BseStream 인터페이스를 부모로 갖고
 *   자식 인터페이스들이 상속 관계를 이루고 있다.
 *   
 * * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 컬렉션(배열)의 저장 요소들을 하나씩 참조해서
 *   람다식으로 처리할 수 있도록 새주는 반복자
...
public class Create {
	...
	public void method2() {
		...
		stream.forEach(name -> System.out.println(name)); // 나열된 이름들을 세로로 나열
