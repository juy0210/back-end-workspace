/*
     ...
     - 조회 결과는 하나의 결과물(RESILT SET)으로 나옴
     - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
       ...
       => 즉, 관계형 데티어베이스에서 SQL 문을 이용한 테이블 간에 "관계"를 맺어 
          원하는 데이터만 조회하는 방법
*/
/*
     1. 내부 조인(INNER JOIN)
     ...
     SELECT 컬럼, 컬럽, ...
     FROM 테이블1
     [INNER(생략 가능)] JOIN 테이블2 ON (테이블1.컬럼명 = 테이블2.컬럼명);
     테이블 = FROM 절에 전체(employee)
     
     - FROM 절에서 기준이 되는 테이블을 기술
     - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매치 시킬 컬럼에 대한 조건을 기술 (ON, USING)
...
-- 직급이 대리인 사원의 사번, 이름, 직급명(job_name), 급여 조회
-- 1) WHERE 구문
...
/*
     ...
     - 단, 반드시 기준이 되는 테이블(컬럽)을 지정해야 한다. (LEFT, RIGHT, FULL)
*/
...
FROM employee
     ...
     -- 3) FULL JOIN : 두 테이블이 가진 모든 행을 조회할 수 있음 - MySQL은 지원X
...
-- 전체 사원의 사원사번, 사원명, 사원부서코드(dept_code), manager_id, 
...
SELECT 
    ...
    m.dept_code "사수부서코드"
FROM employee e -- 별칭을 붙일땐 SELECT, JOIN 에도 붙여야함
...
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM location; -- job_code, local_name

SELECT emp_name, bonus, 
       format((salary + salary * bonus) * 12, 0) 연봉, dept_title, local_name
          -- (salary + salary * bonus) * 12 연봉, dept_title, local_name
FROM employee
    JOIN department ON (dept_code = dept_id)
    JOIN location ON (location_id = local_code)
WHERE bonus is not null;
...
-- 6. 각 부서멸 총 급여의 합이 1000만원 이사아인 부서명, 급여 합 조회
SELECT * FROM employee; -- dept_code, salary
SELECT * FROM department; -- dept_id, dept_title
...
SELECT * FROM employee; -- emp_id, emp_name, job_code
SELECT * FROM job; -- job_code, job_name
SELECT * FROM sal_grade; -- sal_lavel, min_sal, max_sal
...
SELECT * FROM employee; -- emp_name, job_name, salary, bonus
SELECT * FROM job; -- job_code, job_name
...
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM job; -- job_code, job_name
SELECT * FROM location; -- local_code, local_name, cational_code
SELECT * FROM national; -- national_code, national_name
