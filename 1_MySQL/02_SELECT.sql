/*
    ...
    - DBMS : Database Management System의 약자, 데이터베이스 관리시스템
    
    - 관계형(Relational) 데이터베이스
    1. 가장 대표적인 데티어베이스 시스템
	...
	- SQL 종류
	  ...
	  3. DCL(Data Control Language) : 데이터 제어어
         - DBMS에 대한 보안, 무결성, 복구 등 DBMA를 제어하기 위한 언어
...
-- 입사일(hire_data) 정보만 조회
SELECT emp_name, email, phone, hire_date
FROM employee;

-- 5. employee 테이블의 입사일(hire_data), 직원명(emp_name), 급여(salary) 조회 
...
*/
-- employee 테이블에서 직원명(emp_name), 지원 연봉(salary * 12) 조회
...
/*
     ...
     - 산술연산을 하면 컬럼명이 지저분해진다.
       이때 컬럼명에 별칭을 부여해서 깔끔하게 보여중 수 있다. 
	 - 별칭을 부여할 때 띄어쓰기 혹은 특수문다가 포함될 경우 반드시 끈따옴표(")로 감싸준다.
*/
SELECT 
    emp_name 직원명, 
    salary "직원 월급", /* 띄어쓰기를 할려면 ""안에 작성 */
    salary * 12 as 연봉
FROM employee;
...
/*
    ...
    !=, <> : 같지 않은지 비교 연산자
*/
-- employee에서 부서코드가(dept_code)가 'D9'인 사원들만 조회 (이때, 모든 컬럼 조회)
SELECT *
FROM employee
WHERE dept_code = 'D9';
...
/*
    BETWEEN AND
    -조건식에서 사용되는 구문
    - 몇 이상 몇 이하인 번위에 대한 조건을 제시할 때 사용되는 연산자
...
/*
     ...
     비교대상컬럼 LIKE '__문자' => 비교대상컬럽값에 문자앞에 무조건 두글자가 올 경우 조회
     비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자 앞과 뒤에 무조건 한글자씩 올 경우 조회
*/
-- 사원들 중 성이 전씨인 사원들이 사원명, 급여, 입사일 조회
...
-- 첫번째 값은 오프셋(offset, 0부터 시작) 시작 생을 지정
-- 두번째 값은 반호나할 최대 행 수를 지정
SELECT emp_name, salary
FROM employee
ORDER BY salary DESC
LIMIT 10 OFFSET 0; -- = LIMIT 0, 10;
...
-- 3. 연봉(보너스포함x)이 4000만원 이상이고 보너스를 받이 않은 사원들의 사번, 사원명, 급여, 보너스 조회
SELECT emp_id, emp_name, salary, bonus
FROM employee
WHERE salary * 12 >= 40000000 AND bonus IS NULL;

-- 4. 입사일이 '1995-01-01' 이상이고 부서배치를 받은 사원들의 
--    사번, 사원명, 입사일, 부서코드 조회(정렬은 먼저 입사일 기준)
...
--    사원들의 사번, 사원명, 급여, 보너스 포함 연봉(별칭부여) 조회 (연봉이 높은 순으로 정렬)
