...
-- '한국'에서 근무하는 사원들이 사번, 이름, 부서명, 급여, 근무국가명 조회
...
-- timestampdiff = 근무년수 // 03_SELECT 참조
-- 현재 년도 - 입사일 = 근무년수
...
/*
     ...
     - 뷰를 통해 조작해게 되면 실제 데이터가 담겨 있는 테이블에 반영됨
*/
...
-- job-code가 primary key! 즉 not null 제약조건에 걸림!
INSERT INTO vw_job
VALUES('인턴');
...
AS SELECT emp_id 사번, emp_name 사원명, emp_no 주민번호, dept_title 부서명
   FROM employee
   JOIN department ON (dept_code = dept_id);
...
-- 202번의 사원의 급여를 400만원으로 변결 (vw_emp 이용해서)
UPDATE vw_emp
SET salary = 4000000
WHERE emp_id = 202;
