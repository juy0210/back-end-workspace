...
SELECT dept_code, format(avg(salary), 0), format(sum(salary), 0)
FROM employee
GROUP BY dept_code;
...
/*
     rollup|cube(컬럽1, 컬럼2) (cube는 MySQL X) - 실제 볼일은 없는 집계 함수
     - 그룹별 산출한 결과 값의 중간 집계를 계산 해주는 함수
     - rollop : 컬럼1을 가지고 다시 중간집계를 내는 함수
     - cube : 컬럽1을 가지고 중간집계도 내고, 컬럼2를 가지고도 중간집계를 냄
...
/*
     ...
     UNION ALL (합지합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행 제거 X)
...
FROM employee
WHERE dept_code = 'D5'
UNION -- 두 쿼리문을 UNION으로 이은다 (1)(2)가 하나의 쿼리임-중복되는 행은 제거
-- (2) 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 급여 조회
SELECT emp_id, emp_name, dept_code, salary
FROM employee
WHERE salary > 3000000
ORDER BY emp_id; -- ORDER BY를 쓸려면 마지막 쿼리문에 작성해야함
