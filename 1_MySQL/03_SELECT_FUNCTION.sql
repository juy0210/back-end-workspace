/*
     함수 : 전달된 컬럼값을 읽어들여서 함수를 실행한 결과를 반환alter
...
/*
     문자 처리 함수
     LENGTH() : 해당 문자열값의 BTYE 길이 수 반환
     - 한글 한 글자 -> 3BYTE
     - 영문자, 숫자, 특수문자 한글자 -> 1BTYE
...
/*
     TRIM(컬럼|'문자열') | TRIM([LEADING|TRAILING|BOTH] 제거하고자 
          하는 문자들 FROM 컬럼|'문자열')
...
/*
     SUBSTR|SUBSTRING(컬럽|'문자열', 시작 위치 값, 추출할 문자 개수)
     - 문자열에서 특정 문자열을 추출해서 반환
*/

SELECT substr('PROGRAMMING', 5, 2); -- RA --5번째부터 2개 반환
SELECT substr('PROGRAMMING', 1, 6); -- PROGRAMMING -- 1번째부터 6개 반환
SELECT substr('PROGRAMMING', -8, 3); -- GRA -- 뒤에서 8번째에서 앞으로 3개 반환alter
...
/*
     LOWER : 다 소문자로 변경하는 문자열 반환
     UPPER : 다 대분자로 변경하는 문자열 반환
*/
...
-- concat
SELECT emp_no, concat(substr(emp_no, 1, 8), "******")
FROM employee;
...
*/
SELECT period_diff(202406, 202411), period_diff(202412, 202406); -- -5
...
/*
     ...
     COALESCE|IFNULL(값, 값이 NULL일 결우 반환할 값)
*/
...
-- 사원명, 직급코드, 기존급여(salary), 인상된 급여 조회
...
FROM employee
ORDER BY 2, 4 DESC; -- 위에 코드 항목들을 번호로 인식 가능 2 = job_code 
...
/*
     ...
     SUM : 해당 컬럼 값들의 통 합계 반환
*/
-- 전체 사원의 총 급여 합 조회
SELECT sum(salary)
FROM employee;

-- 부서코드가 D5인 사원들이 총 연봉(급여 * 12) 조회
...
/*
     MIN : 해당 컬럽 값들 중에 가장 작은 값 반환
     MAX : 해당 컬럽 값들 중에 가장 큰 값 반환
*/
SELECT
...
/*
     ...
     * : 조회 결과에 해당하는 모든 행 개수 반환
     컬럽 : 해당 컬럽값이 NULL이 아닌 행 개수 반환
     distinct 컬럼 : 해당 컬럽값의 중복을 제거한 행 개수 반환
*/
-- 전체 사원 수 조회
SELECT count(*)
FROM employee;
