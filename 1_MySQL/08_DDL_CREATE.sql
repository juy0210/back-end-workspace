/*
     ...
     - MySQL에서 객체(구조) : 테이블(Table), 뷰(View), 인덱스(Index), 프로시저(Procedure)
                          트리거(Trigger), 함수(Function)
*/
/*
     CREATE
     - 객체를 생성하는 구문
     ...
     * 자료형
     1. 문자
         - CHAR / **VARCHAR** : 고정 및 가변 길이 문자, 반드시 크기 지정/VARCHAR를 주로 씀
	 ...
	 5. 이진 자료형
         ...
         --> 실제로는 이미지나 동영상은 따로 관리함 (URL만 문자령으로 저장하는 편)
*/
...
/*
     ...
     - 제약조건은 데이터 무결성 보장을 목정으로 한다.
     - 종류 : NOT NULL, UNIQUE, CHECH, PRIMARY KEY, FOREIGN KEY
*/
/*
     NOT NULL
...
INSERT INTO mem_notnull 
    VALUES(2, 'user02', null, '손정배', '남', null, null, null);    
INSERT INTO mem_notnull 
    VALUES(2, 'user02', 'pass01', '손정배', '남', null, null, null);        
-- >> NOT NULL 제약조선에 위배되어 오류 발생!    

SELECT * FROM member mem_notnull;
...
/*
    ...
    ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 주문번호, 운송장번호, ...
...
SELECT * FROM member;
-- mem_grade 테이블에서 grade_code 2인 데이터 삭제
...
-- employee 테이블에서 사번, 직원명, 급여, 연봉만 저장하는 테이블 employee_copy 테이블로 생성
...
/*
	 자식테이블에서 생성시 외래키 제약조건을 부여할 때 삭제옵션 지정 가능
