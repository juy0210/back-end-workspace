...
DROP INDEX inx_amount_id ON saila.payment;
...
/*
     ...
     DELIMITER ;
*/
-- 사번을 입력받아 해당 사원의 연봉을 계산하고 리턴하고 함수 생성
DELIMITER //
...
-- 사원의 사번, 사원명, 급여, 연복 조회 (정렬 연봉 높은 순)
...
-- employee 테이블에 새로운 행이 INSERT될때마다 '신입사원이 입사했습니다'라는
...
-- 2. 상품 입/출소 상세 이력 테이블 생성 (prodetail)
/*
...
/*
     ...
     1. 개념적 모델링
         - 엔티티(Entity) 추출
         - 엔티티 간의 관계 설정
     2. 논리적 모델링 : ERD(Entity Relatonship Diagram) 툴
         - 정규화 작업 (1 ~ 5)... 대체적으로 3까지만 정규화한다
           --> 너무 쪼개면 join만 많아진다
     3. 물리적 모델링
         - 테이블 실질적으로 구성
*/
